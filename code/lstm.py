...
if __name__ == '__main__':
    ...
    if train:
        ...
        model.fit(tweets, labels, batch_size=batch_size, epochs=100, validation_split=0.1, shuffle=True, callbacks=[checkpoint, reduce_lr])
    else:
        model = load_model(sys.argv[2])
        print(model.summary())
        test_tweets = utils.process_tweets(TEST_PROCESSED_FILE, service_port, test_file=True, get_feature_vector=get_feature_vector)
        test_tweets = [test_tweet[-1] for test_tweet in test_tweets]
